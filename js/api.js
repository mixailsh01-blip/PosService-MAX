...
const API = {
  ...
  async sendPayStatus(establishmentPayload = {}, userData = null, webApp = null) {
    const hookUrl = `${API_BASE_URL}/webhook/PayStatus`;
    const payload = {
      IDREST: establishmentPayload?.IDREST ?? establishmentPayload?.ID ?? establishmentPayload?.id ?? null,
      KК: establishmentPayload?.KК ?? establishmentPayload?.KK ?? establishmentPayload?.Client ?? establishmentPayload?.name ?? null,
      Client: establishmentPayload?.Client ?? establishmentPayload?.name ?? null,
    ...
    try {
      console.log('📤 [API] Отправляем PayStatus:', payload);
      ...
      const result = await response.json().catch(() => null);
      console.log('✅ [API] Ответ PayStatus:', result);
      return result;
    } catch (error) {
      console.error('❌ [API] Ошибка PayStatus:', error);
      return null;
    }
  }
  ...
  async getPersonRights(userId) {
    ...
    try {
      ...
      const праваПоЗаведениям = new Map(заведения.map(з => [String(з.IDREST), {
        role:                 з.ПРАВА?.['Права доступа'] ?? null,
        просмотрСотрудников:  да(з.ПРАВА?.['Просмотр сотрудников']),
        удалениеСотрудников:  да(з.ПРАВА?.['Удаление сотрудников']),
        редактированиеПрав:   да(з.ПРАВА?.['Редактирование прав доступа']),
        счета:                да(з.ПРАВА?.['Счета']),
        анонимныеЗаявки:      да(з.ПРАВА?.['Анонимные заявки']),
      }]));
      return {
        счета:               счетаЗаведения.length > 0,
        просмотрСотрудников: заведения.some(з => да(з.ПРАВА?.['Просмотр сотрудников'])),
        анонимныеЗаявки:     заведения.some(з => да(з.ПРАВА?.['Анонимные заявки'])),
        редактированиеПрав:  заведения.some(з => да(з.ПРАВА?.['Редактирование прав доступа'])),
        счетаЗаведения:      счетаЗаведения.map(з => ({ id: з.IDREST, name: з.КК })),
        заведенияБезДоступа: заведенияБезДоступа.map(з => ({ id: String(з.IDREST), name: з.КК })),
        запретПросмотраСотрудников: запретПросмотра,
        праваПоЗаведениям,
      };
    } catch (error) {
